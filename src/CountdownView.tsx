...
const CountdownView = component("CountdownView", function(interactions, props) {
  ...
  const clockStream = interval(1000);
  const pauseStream = interactions.get("OnPause").pipe(
    map(_ => true),
    startWith(false),
    scan((a, b) => !a)
  );

  const restartStream = interactions.get("OnRestart").pipe(
    map(_ => true),
    concatMap(
  ...
  const finalStream = clockStream.pipe(
